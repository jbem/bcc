...
"select distinct pays  from capital"|>dbGetQuery(con, statement=_)
...
# Selection avec filtre des informations 
capital= pd.read_sql(
    "select * from capital where pays='Gabon'",
    con
)
...
# Afficher les modalités de variable pays  clause: distinct 
capital= pd.read_sql(
    "select distinct pays  from capital",
    con
)
...
"select AVG(capital) as Moyenne, SUM(capital) as Total from capital"|>dbGetQuery(con, statement=_)|>head()

# statistiques par pays
"select pays, AVG(capital) as Moyenne, SUM(capital) as Total
 from capital
 group by pays"|> dbGetQuery(con,statement=_)|>head()

# Filtre sur les statistiques 
"select pays, AVG(capital) as Moyenne, SUM(capital) as Total
 from capital
 group by pays
 having capital>50000"|>
    dbGetQuery(con,statement=_)|>head()
...
# statistiques du capital par pays et catégories
capital= pd.read_sql(
    "select pays, categorie, AVG(capital) as Moyenne, SUM(capital) as Total " 
    "from capital group by pays, categorie",
    con
)
