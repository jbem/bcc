...
L'installation de R et Python se fait en général à partir des programme d'installation fournit par les sites oficiels des deux langages (www.r-project.org et www.phyton.org). Pour une utilisation lus efficace, il existe des outils plus performants notamment pour gestion des projets complexes (Rstudio, PyCharm, Positron, VS Code). Ces utils permettent de faire simulatenement les deux. Pour cette formation nous allons utiliser [Positron](https://positron.posit.co)  
...
#| echo: false
    ...
    )|>kable(col.names=c("Commande","Fonction"))|>pack_rows("LMD",1,4)|>pack_rows("LDD",5,6)|>pack_rows("DCL",7,8)
...
"create table if not exists capital (
    ...
    capital interger not null,
    bilan   integer not null,
...
n<-"select count(*) from capital"|>
    dbGetQuery(con,statement=_)|>unlist() # Nb de tuples dans la table

if(n==0){
    library(readxl)
    capdata<-read_xlsx("../Regdata.xlsx")
    nobs<-dim(capdata)[1]

    for (i in 1:nobs){
        req<-paste0(
            "INSERT INTO capital (id_banque, capital, bilan, categorie, pays)
            VALUES (",capdata[i,1],",",capdata[i,2],",",capdata[i,3],",'",capdata[i,4],"','",capdata[i,5],"')"
        )
...
Nous allons maintenant aborder la commande **SELECT**
