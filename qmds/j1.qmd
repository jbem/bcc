...
La gestion des versions sur un document ou un ensemble de fichiers et dossiers n'est pas évidente. Une des pratiquex reternue est de nommer le fichier différemment selon les modifications apprortées. Des logiciels comme MS World offrent un mode **suivi des modifications**. Cependant, ces approches sont inefficaces avec de nombreux risques lorsques les fichiers à gerer sont nombreux. Il n'y a pas par exemple ce suivi pour des fichiers pdf. La collaboration peut être délicate si de nomreuses personnes contribuent au projet. 
...
**Dépôt (Repository)** : C'est le dossier contenant l'ensemble des fichiers du projet. Pour cette formation, le repo git aura la structure ci-dessous  :
...
Supporsons que l'on souhaite calculer le ratio **NPL non provisionnées sur fonds propres** c'est un des indicateur de stablité financière du FMI. Les informations se trouvent dans deux tables. Il faut alors recourir au mecanisme de **jointure** de SQL. Les jointures se font sur un certains nombres de champs. 
