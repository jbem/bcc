...
- Le langage R [@ihaka996 ] est une implémentation du langage **S**, développé par **John Chambers**. Ce langage a été primé par l'ACM avec cette citation : 
...
- Le langage Python quant à lui est apparu cinq ans plus tôt en 1991, développé par **Guido van Rossum** aux Pays-Bas. Il est aujourd'hui le logiciel le plus utilisé pour l'IA. 

- Ce deux logiciels, se distinguent par leur flexibilité et leur interactivité. En effet, les logiciels comme SAS et SPSS ont un modèle d'analyse différée. Avec ce modèle, soumet une tâche en fournissant des données et les instructions, le logiciel effectue les analyses et imprime les informations susceptibles d'intéresser l'utilisateur qui scrute ces résultats pour extraire ce qui l'intéresse, et est limité aux processus proposés par le logiciel. 
...
L'installation de R et Python se fait en général à partir des programmes d'installation fournit par les sites officiels des deux langages (www.r-project.org et www.python.org). Pour une utilisation plus efficace, il existe des outils plus performants notamment pour gestion des projets complexes (Rstudio, PyCharm, Positron, VS Code). Ces outils permettent de faire simultanément les deux. Pour cette formation, nous allons utiliser [Positron](https://positron.posit.co)  

**La fin du copier-coller ?** Une autre force ces outils, c'est de permettre la _recherche reproductible_. Traditionnellement, pour rédiger un rapport d'analyse, on utilise un logiciel de traitement de texte, on réalise les calculs et les graphiques ailleurs et souvent par copier-coller, les résultats sont intégrés au rapport. Avec R et Python, il est assez aisé de générer des rapports dynamiques. C'est à ce stade qu'intervient [**Quarto**](https://quarto.org)  

Les deux langages sont extensibles avec une multitude de modules. Toutefois, les approches travail sont différentes. Avec R, vous chargez les modules donc vous avez besoin une fois qu'ils sont installés. C'est aussi le cas avec Python, mais avec une option supplémentaire, **les environnements virtuels**. Un environnement est un espace de travail isolé qui contient sa propre installation de bibliothèque pour un projet donnée. Ils peuvent être créés avec un outil nommé **conda** ou l'outil par défaut **venv**. 

Sous Windows le code ci-après permet de créer et d'activer un environnement virtuel ;
...
L'environnement est alors créé dans le dossier dans lequel la commande création est appelée, et la seconde commande l'active. 

## Introduction au versionning : ![](../Git-Logo-1788C.png){height="1.2em" style="vertical-align: middle;"} 


La gestion des versions sur un document ou un ensemble de fichiers et dossiers n'est pas évidente. Une des pratiques retenue est de nommer le fichier différemment selon les modifications apportées. Des logiciels comme MS World offrent un mode **suivi des modifications**. Cependant, ces approches sont inefficaces avec de nombreux risques lorsque les fichiers à gérer sont nombreux. Il n'y a pas par exemple ce suivi pour des fichiers pdf. La collaboration peut être délicate si de nombreuses personnes contribuent au projet. 

>**Sans Git**  
>rapport_v0.docx 
>rapport_v1.docx 
>rapport_v1_reluDG.docx 
>rapport_final.docx 

Un logiciel de gestion de version comme **Git** permet de :

- conserver l'historique des modifications ;
- revenir à une version antérieure ;
- travailler à plusieurs ;
- de suivre les modifications sur de nombreux fichiers de nature différente. 

Vous pouvez travailler avec Git en local, ou mieux transférer vos données en ligne à travers des plateformes comme [**Github**](www.github.com),
 [**GitLab**](about.gitlab.com) ou encore [**Bitbucket**](www.bitbucket.org). On peut travailler avec Git de plusieurs manières.  En obtenant le programme
 depuis le site officiel, vous aurez un outil graphique et un outil de commande en ligne. La plupart des IDE (comme Positron) permette de travailler avec
 Git directement. Les participants qui n'ont pas de compte github vont commencer par en créer. 

### Concepts de base 

**Dépôt (Repository, repo)** : C'est le dossier contenant l'ensemble des fichiers du projet. Pour cette formation, le repo git aura la structure ci-dessous :
...
Le repo est mis en place en se plaçant dans la racine du dossier training et en exécutant la commande

````bash
    # Initialisation du repo
    git init

    # Par défaut git crée une branche master : renommer par exemple main
    git branch -M main

    # Dire de suivre les fichiers index.qmd et autres
    git add index.qmd
    git add scripts/main.R
    git add scripts/imports.py
...
Un commit est un instantané du projet.  

````bash
git commit -m "Calcul du ratio CET1"
````

**Historique**

C'est la liste de tous les commits du projet. La syntaxe est :
...
Pour aller à un état précédent, utilisez la commande

````bash

    git checkout <id-du-commit>
    # git log --online  affiche les id des commits (7 chiffres en jaune au debut)
...
> **Git** est à l'analyse de données ce qu'un système de contrôle interne est à une banque : il permet de tracer, documenter et sécuriser l'évolution des travaux. GitHub ajoute une dimension collaborative en facilitant le partage et la revue des analyses.
...
Dans R, avec l'opérateur pipeline (**|>** package **base** ou **%>%** du package **magrittr**), permet le calcul en une seule instruction $$v_4 \leftarrow x |> t()|>h()|>f()|>g()$$ Avec Python, dispose d'une fonction **.pipe()** que nous verrons.
...
Les fonctions sont importantes avec ces deux langages, car permettent une très grande flexibilité. Avec R, elles sont définies avec le mot clé **function** suivie de la liste des arguments et du corps de la fonction entre deux accolades. Avec Python, la fonction est définie avec **def**.
...
## La recherche reproductible : ![](../quarto.png){height="1em" style="vertical-align: middle;"} 
...
> Copiez ce fragment dans le fichier *index.qmd* du **repo training** 
...
> Copiez ce bloc et complétez le fichier index.qmd. 
...
:::{.notes}
Avec Excel, toute les informations sont stockées dans un seul fichier. 
:::

Les concepts suivants sont fondamentaux :

- **Table (ou relation)** : représente une entité avec ses informations (banque, prêt, transfert);
...
"PRAGMA foreign_keys=ON"|>dbExecute(con,statement=_)
...
Chargeons la base avec les fichiers Excel
...
if(n==0){
    ...
    for (i in 1:nobs){
        req<-paste0(
            "INSERT INTO banque (id_banque, id_pays, categorie, pays) VALUES (",bks[i,1],",",bks[i,2],",",bks[i,3],",'",bks[i,4],"')"
        )
...
n<-"select count(*) from capital"|> dbGetQuery(con,statement=_)|>unlist()

if (n==0){
    capital<-readxl::read_xlsx("../capital.xlsx")
...
n = pd.read_sql_query("SELECT COUNT(*) AS n FROM loansdeposits",con).iloc[0, 0]

if n==0:
    loansdeposits = pd.read_excel("../loansdeposits.xlsx")
    loansdeposits.to_sql("loansdeposits",engine,if_exists="replace",index=False)

n = pd.read_sql_query("SELECT COUNT(*) AS n FROM liquidity",con).iloc[0, 0]
if n==0:
...
Cette commande lire les informations d'une ou plusieurs tables. Nous allons illustrer par des exemples. Les commandes peuvent être lancées en utilisant **Positron** ou depuis R ou Python. 
...
La commande **SELECT** ne se limite pas à afficher les données, elle permet d'obtenir des résumés (Moyenne, Maximum, Minimum, Somme...) sur l'ensemble de la table ou par groupe. Nous allons voir ce volet analytique. 
...
Dans une BDD, toutes les infirmations ne sont pas stockées dans une seule table. Ces bases peuvent avoir des dizaines, voir des milliers de tables liées entre elles. La BDD que nous avons créée est affichée ci-après :

![](../dbBCC.png)

Supposons que l'on souhaite calculer le ratio **NPL non provisionnés sur fonds propres**, c'est un des indicateurs de stabilité financière du FMI. Les informations se trouvent dans deux tables. Il faut alors recourir au mécanisme de **jointure** de SQL. Les jointures se font sur un certain nombre de champs. 
...
#| echo: false
    ...
    dplyr::tibble(
        colonne =c("INNER JOIN","LEFT JOIN","RIGH JOIN","FULL OUTER JOIN","CROSS JOIN","SELF JOIN"),
        role=c("Conserve les informations pour lesquelles champs jointures sont identiques","Conserve toute la tableau de gauche et lie les informations correspondantes de la table à droite","Idem à droite","Combine les deux tables  en ajoutant des NULL en cas de non correspondance","Produit cartésien","Jointure d'une table avec elle")
    )|>kable(format= "html",col.names=c("Option de jointure","Fonction"))

```

L'option **LEFT JOIN** est la plus utilisée en exploration ou consultation d'un BDD. Nous allons illustrer en calculant notre ISF. 
...
"select pays, c.annee, c.mois, (sum(npl)-sum(provisions))/sum(fpr) as ISF 
from capital c
    left join loansdeposits l on c.id_banque=l.id_banque and c.id_pays=l.id_pays and 
                                c.annee=l.annee and c.mois=l.mois 
    left join banque b on c.id_banque=b.id_banque and c.id_pays=b.id_pays 
group by c.annee, c.mois, pays"|>dbGetQuery(con, statement=_)|>head()
